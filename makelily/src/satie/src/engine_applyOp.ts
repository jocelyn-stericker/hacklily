 ...
 */
export default function applyOp(
  preview: boolean,
  measures: IMeasure[],
  factory: IFactory,
  op: IAny,
...
export function applyMeasureOp(
  measures: IMeasure[],
  factory: IFactory,
  op: IAny,
  doc: Document,
  ...
  if (op.ld !== undefined && op.p.length === 1) {
    ...
    invariant(
      op.ld.uuid === measures[measureIdx].uuid,
    ...
    measures.slice(measureIdx).forEach(measure => {
      ...
      if (!isNaN(parseInt(measure.number, 10))) {
        ...
        console.warn(
          "Cannot change bar number for invalid measure number ",
          measure.number,
        );
      }
    });
  ...
  invariant(
    ok,
    `Invalid operation type for applyMeasureOp's context: ${JSON.stringify(
      op,
    )}`,
  );
}
