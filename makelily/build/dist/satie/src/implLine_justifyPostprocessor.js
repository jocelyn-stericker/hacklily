...
function justify(options, bounds, measures) {
    ...
    var x = bounds.left + reduce(measures, function (sum, measure) { return sum + measure.width; }, 0);
    // Check for underfilled bars
    var underfilled = map(measures, function (measure, idx) {
        var attr = measures[idx].attributes;
        var firstPart = scoreParts(options.header.partList)[0].id;
        var divs = barDivisions(attr[firstPart][1]);
    ...
    var expandableCount = reduce(measures, function (memo, measure$, idx) {
    ...
    forEach(measures, function (measure, measureIdx) {
        ...
        if (options.fixedMeasureWidth) {
            var expandable_1 = times(maxIdx, function (j) {
                var expand = false;
                for (var i = 0; i < measure.elements.length; ++i) {
                    if (measure.elements[i][j].expandPolicy !== "none") {
                        expand = true;
