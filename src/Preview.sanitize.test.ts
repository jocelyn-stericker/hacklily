 ...
 * ALLOW_UNKNOWN_PROTOCOLS: true })`. This test locks in the behaviors that
 * matter for rendering so a future DOMPurify bump or config change can't
 ...
 *  4. Dangerous content DOMPurify must strip is stripped (<script>, event
 *     handlers, javascript: URIs), so the iframe (which is NOT sandboxed) stays
 *     safe.
 */
...
const PREVIEW_CONFIG = { ALLOW_UNKNOWN_PROTOCOLS: true } as const;
...
describe("Preview DOMPurify sanitization", () => {
  ...
  test("strips javascript: URIs", () => {
    ...
    // But the textedit: protocol (an unknown, but safe, protocol) is kept.
