...
export function parseClef(clefLy: string): Clef {
  ...
  return buildClef((clef: IClefBuilder): IClefBuilder => clef
...
export function parseKeySig(keyLy: string): Key {
  ...
  return buildKey((key: IKeyBuilder): IKeyBuilder => key
    .fifths(fifths)
    .mode(mode));
}
...
export function parseTime(timeLy: string): Time {
  ...
  return buildTime((time: ITimeBuilder): ITimeBuilder => time
    .beatTypes(beatTypes)
    .beats(beats));
}
