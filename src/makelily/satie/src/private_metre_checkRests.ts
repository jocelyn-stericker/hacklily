...
function _getValidSubBeatLengths(
  quantumPerBeats: number,
...
class RestSolver {
  ...
  constructor(ruleBeats: number, ruleBeatType: number, restRules: RestRules) {
  ...
  checkRests(divisions: number, song: string, options: IOptions): string {
    ...
    const validSubBeatLengths = _getValidSubBeatLengths(
      quantumPerBeats,
      this._ruleBeats,
      options.dotsAllowed,
    );
    const validNonDotSubBeatLengths = _getValidSubBeatLengths(
      quantumPerBeats,
      this._ruleBeats,
      false,
    );
          ...
          if (this.isSimple()) {
            ...
            subbeatOptions: for (
              ...
              if (matchedLength >= subbeatLength) {
                ...
                // beat as well.
                const isDotted =
                  validNonDotSubBeatLengths.indexOf(subbeatLength) === -1;
...
const REST_RULES_1: RestRules = Object.freeze(["r", "*"]);

const REST_RULES_2: RestRules = Object.freeze(["r_", "r.", ".r", "*.", ".*"]);
