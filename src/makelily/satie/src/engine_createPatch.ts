...
export class PartBuilder {
  ...
  voice(
    voiceID: number,
    builder: (build: VoiceBuilder) => VoiceBuilder,
    idx?: number,
  ): this {
    const voice = this._part ? this._part.voices[voiceID] : null;
    invariant(!this._part || Boolean(voice), "invalid voice");
    this._patches = this._patches.concat(
      builder(new VoiceBuilder(voice, this._document, idx)).patches.map(
        _prependPatch("voices", voiceID),
      ),
    );
    return this;
  }
  ...
  staff(
    staffID: number,
    builder: (build: StaffBuilder) => StaffBuilder,
    idx?: number,
  ): this {
    const staff = this._part ? this._part.staves[staffID] : null;
    invariant(!this._part || Boolean(staff), "invalid staff");
    this._patches = this._patches.concat(
      builder(new StaffBuilder(staff, this._document, idx)).patches.map(
        _prependPatch("staves", staffID),
      ),
    );
    return this;
  }
...
export class MeasureBuilder {
  ...
  part(partID: string, builder: (build: PartBuilder) => PartBuilder): this {
    ...
    this._patches = this._patches.concat(
      builder(new PartBuilder(part, this._document)).patches.map(
        _prependPatch("parts", partID),
      ),
    );
    return this;
  }
...
export class DocumentBuilder {
  ...
  insertMeasure(
    ...
    this._patches = this._patches.concat(
      builder(new MeasureBuilder(null, this._doc)).patches.map(
        _prependPatch(uuid),
      ),
    );
    return this;
...
function getMutationInfo(document: Document, patches: IAny[]) {
  ...
  patches.forEach(patch => {
    ...
    invariant(
      part,
      `part ${patch.p[2]} should exist in measure ${measureUUID}`,
    );
    if (patch.p[3] === "staves") {
      return;
    }
    ...
    invariant(
      voice,
      `expected to find voice ${patch.p[4]} in part ${patch.p[2]} in measure ${measureUUID}`,
    );
    ...
    if (patch.p.length === 6) {
      if (patch.li) {
        ...
        invariant(
          isInteger(spliceIdx) && !isNaN(spliceIdx),
          "Expected an integer",
        );
        if (spliceIdx === 0) {
          start = 0;
        ...
        let newInfo = new ModelMetreMutationSpec({
          ...
          rest: !!theRest,
          forced:
            theRest && typeof theRest !== "boolean" && "_force" in theRest,
          beam: b,
          touched: true,
        });
...
function fixBarlines(_doc: Document, patches: IAny[]): IAny[] {
  // XXX: FIXME
...
function addBeams(document: Document, patches: IAny[]): IAny[] {
  ...
  forEach(elementInfos, (voiceInfo, key) => {
    ...
    voiceInfo.forEach((elInfo, originalIdx) => {
      ...
      const isCandidate =
        countToIsBeamable[elInfo.newCount] &&
