...
export function voiceToRestSpec(
    ...
    (restSpec: IRestSpec, model: ModelMetreMutationSpec) => {
      ...
      const restsAtEnd =
        model.previousDivisions > divCount
          ? "r" +
            times(model.previousDivisions - divCount - 1, () => "_").join("")
          : "";
      const modelsToKill = restSpec.modelsToKill;

      if (divsToSuppress > 0) {
        const extraSong =
          divCount > divsToSuppress
      ...
      const models = restSpec.models
        .concat(times(divCount, () => model))
        .concat(restsAtEnd.split("").map(() => null));
  ...
  const restsToAdd =
    (totalDivisions - (spec.song.length % totalDivisions)) % totalDivisions;
...
function _cleanupRests(pattern: string, time: Time) {
  ...
  const next = () =>
    checkRests(ts, pattern.length, pattern, { dotsAllowed: true });
  ...
  for (let status = next(); status !== "GOOD"; status = next()) {
    ...
    invariant(
      parseInt(cmd[1], 10) === pattern.length,
      "Unexpected length change from %s to %s",
      cmd[1],
      pattern.length,
    );
    const patch = cmd[2];
...
export function simplifyRests(
  ...
  for (let i = 0; i < originalSpec.song.length; i += totalDivisions) {
    cleanRestPattern += _cleanupRests(
      originalSpec.song.slice(i, i + totalDivisions),
      attributes.time,
    );
  }
  ...
  function killModel(model: ModelMetreMutationSpec) {
    ...
    if (!model.newDivisions) {
      ...
      invariant(
        segment.indexOf(model) > -1,
        "Model must be present in segment",
      );
      patches.push({
        ld: model.toSpec(),
        p: [currIdx],
