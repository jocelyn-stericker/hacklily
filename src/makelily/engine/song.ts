 ...
 */
export default class Song extends Component<IProps, IState> implements ISong {
    state: IState = {
        document: null,
        memo: null,
        factory: null,
    ...
    private _createPatch(isPreview: boolean, patchSpecs: IPatchSpec[]) {
        let patches: IAny[] = patchSpecs.reduce((array, spec) => {
            ...
            } else if (PrivatePatches.verify(spec)) {
        ...
        return new PrivatePatches(this._docPatches.slice(), isPreview, _SongTSPermission);
    }
...
/**
 * Proof that we are in song.ts. Used to prevent protected constructors from being
 * accidentally accessed.
 */
const _SongTSPermission = {};

class PrivatePatches {
    isPatches = true;

    content: IAny[];
    isPreview: boolean;

    constructor(content: IAny[], isPreview: boolean, __permission: {}) {
        if (__permission !== _SongTSPermission) {
            throw new Error("PrivatePatches() is private.");
        }
    ...
    static verify(patches: any): patches is PrivatePatches {
        // You can still do Object.create(Object.getPrototypeOf(somePrivatePatch)),
        // but we're doing this only to stop accidental use.
        return patches && patches instanceof PrivatePatches &&
            Object.getPrototypeOf(patches) === PrivatePatches.prototype;
    }
}
