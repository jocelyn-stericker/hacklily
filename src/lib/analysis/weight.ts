...
export function weight(
  ...
  const { sampleRate, freqStepHz, firstBinHz } = params
  const frameStepSec = params.timeStepSamples / sampleRate
  ...
  const hop = Math.max(1, Math.round(TILT_HOP_SEC / frameStepSec))
  for (let fi = 0; fi < frames.length; fi += hop) {
    const frame = frames[fi]!
    if (!frame.pitchDetected) continue

    let eLow = 0
    let eHigh = 0
    let cNum = 0
    let cDen = 0
    for (let k = 0; k < frame.spectrum.length; k++) {
      ...
      const p = 10 ** (int8ToDb(frame.spectrum[k]!) / 10) / deEmphasisGain(freq)
      if (freq >= TILT_LOW_HZ[0] && freq < TILT_LOW_HZ[1]) eLow += p
      else if (freq >= TILT_HIGH_HZ[0] && freq < TILT_HIGH_HZ[1]) eHigh += p
      cNum += freq * p
      cDen += p
    }
    if (eLow > 0 && eHigh > 0) tilts.push(10 * Math.log10(eHigh / eLow))
  }

  return tilts.length
    ? // median
      (tilts.sort((ta, tb) => ta - tb)[Math.floor(tilts.length / 2)] ?? null)
    : null
}
