...
export default function Tests(props: { testId?: string }) {
  ...
  if (filter) {
    const showMoreLink =
      filter.length > 1
        ? `/satie/tests/${filter.substr(0, filter.length - 1)}/?mode=${
            isSingleLine ? "singleline" : "page"
          }`
        : `/satie/tests?mode=${isSingleLine ? "singleline" : "page"}`;
    const swapModeLink = `/satie/tests/${filter}?mode=${
