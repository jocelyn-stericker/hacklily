...
2. **Warm Scheme server**: inside each container, `lily-server.scm` keeps a LilyPond process listening on TCP 1225. `render-impl.bash` connects per request, writes the source, and reads until EOF (the framing signal). The outer bash timeout is set to 500ms less than the Rust harness render timeout (passed into the container as `HACKLILY_RENDER_TIMEOUT_MS`; 8s → 7.5s in production) so the container emits its own error (with partial logs) before the harness kills it.
